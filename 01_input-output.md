...
*Для написания программы в IDLE нужно создать файл (ctrl^n или File - New File). Пишем программу в пустом окошке программы. Для запуска программы нажимаем F5 или Run - Run Module. Перед запуском программы IDLE потребует сохранить файл. Программа исполняется в `терминале`, это - окошко со стрелочками `>>> `, в котором мы код не пишем.*
